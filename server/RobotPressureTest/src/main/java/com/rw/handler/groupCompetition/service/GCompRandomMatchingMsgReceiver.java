...
public class GCompRandomMatchingMsgReceiver implements MsgReciver {
	...
	public boolean execute(Client client, Response response) {
		...
		try {
			...
			if (rsp == null) {
				RobotLog.fail("GroupCompetitionHandler[send] requestRandomMatching转换响应消息为null");
				return false;
			}
			if (!rsp.getResultType().equals(GFResultType.SUCCESS)) {
				RobotLog.fail("GroupCompetitionHandler[send] requestRandomMatching服务器返回不成功 ");
				return true;
			}

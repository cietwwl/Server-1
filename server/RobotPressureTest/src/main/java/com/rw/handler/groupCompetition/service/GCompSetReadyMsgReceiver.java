...
public class GCompSetReadyMsgReceiver implements MsgReciver {
	...
	public boolean execute(Client client, Response response) {
		try {
			...
			if (rsp == null) {
				RobotLog.fail("GroupCompetitionHandler[send] sendSetReady转换响应消息为null");
				return false;
			}
			if (!rsp.getResultType().equals(GCResultType.SUCCESS)) {
				RobotLog.info("GroupCompetitionHandler[send] sendSetReady服务器返回不成功，提示信息：" + rsp.getTips());
				return true;
			} else {
				RobotLog.info("请求准备状态响应成功，userId：{}" + client.getUserId());
				return true;
			}

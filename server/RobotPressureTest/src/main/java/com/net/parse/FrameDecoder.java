...
public class FrameDecoder extends ByteToMessageDecoder {
	...
	public static final AttributeKey<CLIENT_PROTOCOL_TYPE> key = AttributeKey.valueOf("clientProtocol");
	//
	// // 控制变量
	...
	protected void decode(ChannelHandlerContext ctx, ByteBuf in, List<Object> out) throws Exception {
		...
		if ((tag0 & 0XFF) == 0XF0 && (tag1 & 0XFF) == 0XF0 && (tag2 & 0XFF) == 0XF1) {
			...
			if (bb != 0) {
				...
				if (orgLen != trans.length) {
					System.out.println("WARNING: decompressed size not consistent, client say is " + orgLen + ",server found is " + trans.length);
				}
			} else {
				trans = arr;
			}
		 ...
		 * 
		 * in.markReaderIndex(); byte [] b = new byte[4]; in.readBytes(b); ByteBuffer bf = ByteBuffer.wrap(b);
		 * 
		 * bf.order(ByteOrder.LITTLE_ENDIAN); int size = bf.getInt();
		 * 
		 * if (size<0 || size>MAX_SIZE) { throw new CorruptedFrameException("wrong size of packet"); }
		 * 
		 * if (in.readableBytes() < size) { in.resetReaderIndex(); return; }
		 * 
		 * out.add(in.readBytes(size));
		 */
	}

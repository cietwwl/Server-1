...
public final class GroupMemberDataHolder {
	...
	public GroupMemberDataHolder(String groupId) {
		...
		if (mapItemStore.getSize() <= 0) {// 没有任何成员
			return;
		}

		// 成员信息处理
		Enumeration<GroupMemberData> memberEnum = mapItemStore.getEnum();// 帮派中所有成员的信息
	...
	public GroupMemberData getMemberData(String userId, boolean isApply) {
		...
		return mapItemStore.getItem(memberId);
	}
	...
	public GroupMemberData addMember(String userId, GroupMemberData memberData, boolean isAddApply) {
		...
		// 放入到数据库
		mapItemStore.addItem(memberData);
		return memberData;
	}
	...
	public void removeMemberData(String userId, boolean isApply) {
		...
		mapItemStore.removeItem(memberId);
	}
	...
	public void synMemberData(Player player, boolean isApply, int version) {
		if (isApply) {// 发送申请成员的信息
			...
			if (newVersion != version) {
				ClientDataSynMgr.synDataList(player, getApplyMemberSortList(GroupMemberHelper.applyMemberComparator), applyMemberSynType,
						eSynOpType.UPDATE_LIST, newVersion);
			}
		} else {// 正式成员

...
public final class GroupBM {
	...
	public synchronized static Group create(Player player, String groupName, String icon, int defaultValidateType, int defaultApplyLevel) {
		...
		// 放入成员
		group.getGroupMemberMgr().addMemberData(player.getUserId(), newGroupId, player.getUserName(), player.getHeadImage(), player.getTemplateId(),
				player.getLevel(), player.getVip(), player.getCareer(), GroupPost.LEADER_VALUE, 0, now, now, false);
	...
	public synchronized static void dismiss(final String groupId) {
		...
		PlayerTask applyPlayerTask = new PlayerTask() {
			...
			@Override
			public void run(Player player) {
				player.getUserGroupAttributeDataMgr().updateDataWhenRefuseByGroup(groupId);
			}
		};

...
public class GameLoginHandler {
	...
	public ByteString gameServerLogin(GameLoginRequest request) {
		...
		if (userAccount == null) {
			...
			if (user == null) {
				if (GameManager.isWhiteListLimit("")) {
				...
				return response.build().toByteString();
			} else if (GameManager.isWhiteListLimit(user.getUserId())) {
				response.setError("该区维护中，请稍后尝试，");
				response.setResultType(eLoginResultType.ServerMainTain);
				return response.build().toByteString();
	...
	public ByteString createRoleAndLogin(GameLoginRequest request) {
		...
		// author: lida 平台已经判断
		/**
		 * TableAccount userAccount = accountBM.getByAccountId(accountId); if
		 * (userAccount == null) {
		 * response.setResultType(eLoginResultType.FAIL);
		 * response.setError("账号不存在"); return response.build().toByteString(); }
		 * else if (!password.equals(userAccount.getPassword())) {
		 * response.setResultType(eLoginResultType.FAIL);
		 * response.setError("账号或者密码不对"); return
		 * response.build().toByteString(); } else
		 */
		{
			String clientInfoJson = request.getClientInfoJson();
	...
	private boolean notifyPlatformPlayerLogin(int zoneId, String accountId, Player player, int lastZoneId) {
		try {
			if (lastZoneId != zoneId) {
				...
				requestAction.pushParams(UserBaseDataResponse.class,
						userBaseDataResponse);
				Object result = requestAction.remoteCall(
						GameManager.getPlatformUrl(),
						"com.rw.netty.http.requestHandler.PlayerLoginHandler",
						"notifyPlayerLogin");

...
public class SecretHandle {
	...
	private TagFightRoleInfo setTagChangeList(TagFightRoleInfo.Builder tagRoleInfo, List<TowerHeroChange> palyerChangeMap) {
		...
		if (palyerChangeMap != null) {
			for (int i = 0; i < palyerChangeMap.size(); i++) {
				...
				heroChange.setUserId(palyerChangeMap.get(i).getRoleId());
				heroChange.setReduceLife(palyerChangeMap.get(i).getReduceLife());
				heroChange.setReduceEnegy(palyerChangeMap.get(i).getReduceEnegy());
				heroChange.setIsDead(palyerChangeMap.get(i).getIsDead());
	 ...
	 */
	private ArmyInfo getSecretEnemy(PlayerIF targetPlayer, List<TowerHeroChange> heroChangeList, List<String> heroIdList) {// player 信息转为TagTowerEnemyInfo

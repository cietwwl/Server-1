...
public class GameManager {
	...
	private static void initServerPerformanceConfig(){
		...
		try {
			...
			e.printStackTrace();
			//使用默认配置
			ServerPerformanceConfig config = new ServerPerformanceConfig(3000, 10000, 20000);
			performanceConfig = config;
		}
	}
	...
	public static boolean isWhiteListLimit(String userId) {
		boolean serverClose = ServerStatusMgr.getStatus() == ServerStatus.CLOSE;
		if(serverClose && StringUtils.isBlank(userId)){
			return true;
		}
		boolean whiteListON = ServerStatusMgr.isWhilteListON();
		boolean userInWhiteList = ServerStatusMgr.getWhiteList().contains(userId);

...
public interface PlayerExtPropertyCreator<T extends PlayerExtProperty> {
	 ...
	 * @return
	 */
	public List<T> firstCreate(PlayerPropertyParams params);

	/**
	 * <pre>
	 * 在数据库已有记录的情况下，会在忒定时间点(或事件)进行检查是否需要创建，如玩家登录
	 ...
	 * @return
	 */
	public List<T> checkAndCreate(PlayerExtPropertyStore<T> store, PlayerPropertyParams params);
}

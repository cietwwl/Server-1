...
public class TaoistHandler {
	...
	public ByteString upgradeTaoist(Player player, TaoistRequest req) {
		...
		if (newLevel > maxCfgLevel){
			GameLog.info("道术", player.getUserId(), "升级道术不能超过最大配置等级,当前等级:"+currentLvl+"升级次数:" + upgradeCount + ",最大配置等级" + maxCfgLevel, null);
		}
		
		if (newLevel > plvl){
			GameLog.info("道术", player.getUserId(), "升级道术不能超过玩家等级,当前等级:"+currentLvl+"升级次数:" + upgradeCount + ",玩家等级" + plvl, null);
		}
		...
		//检查是否够钱
		int coinCount = consumeHelper.getConsumeCoin(consumeId, currentLvl, newLevel);
		if (coinCount == -1){
		...
		// 验证暴击次数
		int currentSeed = taoistMgr.getRandomSeed();
		int maxCount = helper.generateCriticalCount(currentSeed,taoistMgr.getSeedRange(),tid,upgradeCount);

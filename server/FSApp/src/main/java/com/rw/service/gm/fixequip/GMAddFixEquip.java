...
public class GMAddFixEquip {
	...
	public static void addStarUp(Player player, List<ItemInfo> fillList) {
		...
		for (FixNormEquipStarCfg cfg : cfgList) {
			for (Map.Entry<Integer, Integer> entry : cfg.getItemsNeed().entrySet()) {
				...
				// player.getItemBagMgr().addItem(entry.getKey(),
				// 99999);//各英雄升星特色材料
	...
	public static void addexp(Player player, List<ItemInfo> fillList) {
	...
	public static void addqualityUp(Player player, List<ItemInfo> fillList) {
		List<ItemInfo> list = new ArrayList<ItemInfo>();

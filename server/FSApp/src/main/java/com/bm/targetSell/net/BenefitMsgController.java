...
public class BenefitMsgController{
	...
	private BenefitSystemMsgSender msgSender;
	...
	public void init(String removeIp, int port, int timeoutMillis,int priod){
		...
		msgSender = new BenefitSystemMsgSender(removeIp, port, timeoutMillis);
		FSGameTimerMgr.getInstance().submitMinuteTask(new HeartBeatTask(), priod);
		scanQueue();
	}

				...
				for (ItemDropAndApplyTemplate template : map) {
					...
					for (int i = 0; i < applyInfo.size(); i++) {
						...
						if (memberData == null || !org.apache.commons.lang3.StringUtils.equals(memberData.getGroupId(), groupId)) {
							...
							GameLog.warn(LogModule.GroupCopy.getName(), "GroupCopyMgr[sendMail]", String.format("发放帮派[%s]奖励道具，道具：[%s],接收角色[%s],时间：[%s],"
									+ "发现角色已经不在帮派[%s]内，执行移除操作：[%s]", groupId,
					...
					if (sendMail) {
						send = true;
						//GameLog.info(LogModule.GroupCopy.getName(), "GroupCopyMgr[sendMail]", String.format("发放帮派[%s]奖励道具成功，道具：[%s],接收角色[%s],时间：[%s]", groupId,template.getItemID(), apply.getRoleName(), DateUtils.getDateTimeFormatString(time, "yyyy-MM-dd HH:mm:ss")));

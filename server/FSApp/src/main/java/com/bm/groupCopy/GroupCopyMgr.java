...
public class GroupCopyMgr {
	...
	public GroupCopyResult endFight(Player player, String levelId, List<GroupCopyMonsterSynStruct> mData, List<String> heroList) {
		...
		if (result.isSuccess()) {
			...
			if (mapRecord.getStatus() == GroupCopyMapStatus.FINISH) {
				...
				final String groupID = GroupHelper.getInstance().getUserGroupId(player.getUserId());
				if (groupID.equals("")) {
					return result;
				}
	...
	public static GroupCopyArmyDamageInfo armyInfo2DamageInfo(ArmyInfo info, Player player) {
		...
		String groupName = GroupHelper.getInstance().getGroupName(damageInfo.getPlayerID());
	...
	public GroupCopyResult ApplyOrCancelItem(Player player, GroupCopyCmdReqMsg reqMsg, boolean apply) {
		// 保存下帮派id
		GroupCopyDistIDManager.getInstance().addGroupID(GroupHelper.getInstance().getUserGroupId(player.getUserId()));
				...
				for (ItemDropAndApplyTemplate template : map) {
					...
					for (int i = 0; i < applyInfo.size(); i++) {
						...
						if (memberData == null || (drop.getTime() < memberData.getReceiveTime())) {
							GameLog.warn(
									LogModule.GroupCopy.getName(),
									"GroupCopyMgr[sendMail]",
									String.format("Group ID[%s], item drop time [%s], role[%s] join group time [%s], he can't get item", group.getGroupBaseDataMgr().getGroupData().getGroupId(), DateUtils.getDateTimeFormatString(drop.getTime(), "yyyy-MM-dd HH:mm:ss"), apply.getRoleName(),

...
public class UserFeatruesBuyPowerFour implements IUserFeatruesHandler{
	...
	public RewardBackTodaySubItem doEvent() {
		...
		subItem = ActivityRetrieveTypeHelper.getInstance().doBuyPowerEvent(UserFeaturesEnum.buyPowerFour);
		return subItem;
	}

	@Override
	public RewardBackSubItem doFresh(RewardBackTodaySubItem todaySubItem, Player player, CfgOpenLevelLimitDAO dao) {
		ActivityRetrieveTypeHelper.getInstance().doBuyPowerFresh(todaySubItem, player, dao, UserFeatruesMgr.buyPowerFour);
		return null;
	}

...
public class ActivityRateTypeMgr {
	...
	public int getmultiple(Player player, ActivityRateTypeEnum activityRateTypeEnum){
		ActivityRateTypeItemHolder dataHolder = ActivityRateTypeItemHolder.getInstance();
		if(activityRateTypeEnum == null){
	...
	private boolean isClose(ActivityRateTypeItem ActivityRateTypeItem) {
		...
		if(!isclose){//活动期间的小时区间是否开启
			...
			for(ActivityRateTypeStartAndEndHourHelper timebyhour : cfgById.getStartAndEnd()){
				isclose = hour>=timebyhour.getStarthour()&&hour<timebyhour.getEndhour()?false:true;
				if(!isclose){
					break;
				}
			}			
	...
	private boolean isOpen(ActivityRateTypeCfg ActivityRateTypeCfg) {
		...
		long currentTime = System.currentTimeMillis();
		isopen = currentTime < endTime && currentTime > startTime?true:false;
	...
	public eSpecialItemIDUserInfo getesESpecialItemIDUserInfo(ActivityRateTypeEnum activityRateEnum,eSpecialItemIDUserInfo eSpecialItemIDUserInfo,int expvalue,int coinvalue) {

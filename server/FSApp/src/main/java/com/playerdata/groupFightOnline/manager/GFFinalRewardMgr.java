...
public class GFFinalRewardMgr {
	...
	public void getFinalReward(Player player, int resourceID, String rewardID) throws GFRewardItemException {
		...
		if (item == null)
			throw new GFRewardItemException("指定的奖励不存在");
		if (item.getRewardContent() == null)
			throw new GFRewardItemException("奖励的内容数据有误");
		...
		// }
		ItemBagMgr.getInstance().addItem(player, item.getRewardContent());
		// 领取奖励后删除

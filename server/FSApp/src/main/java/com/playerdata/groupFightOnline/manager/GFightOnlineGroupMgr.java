...
public class GFightOnlineGroupMgr {
	...
	public void dispathchBidOnReward(String groupId) {
		...
		if (totalRateOnGroup > 0) {
			...
			if (totalRateOnGroup > bidCfg.getVictoryMaxRate())
				totalRateOnGroup = bidCfg.getVictoryMaxRate();
			...
			for (GroupMemberDataIF member : groupMem) {
				...
				EmailUtils.sendEmail(member.getUserId(), String.valueOf(bidCfg.getVictoryRewardEmailId()), GFightHelper.itemListToString(victoryReward), String.format(bidonContent, resCfg.getResName(), ratePersonCount));
			}
		}
	}
}

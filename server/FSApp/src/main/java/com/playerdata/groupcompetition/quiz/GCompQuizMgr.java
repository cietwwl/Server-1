	...
	public void createNewQuiz(Player player, Builder gcRsp, int matchId, String groupId, int coin) {
		...
		case FINISH: // 结束
			if (GroupCompetitionMgr.getInstance().getCurrentEventsType() == GCEventsType.FINAL) {
				...
				if (targetAgainst != null && !StringUtils.isEmpty(targetAgainst.getGroupA().getGroupId()) && !StringUtils.isEmpty(targetAgainst.getGroupB().getGroupId()) && StringUtils.isEmpty(targetAgainst.getWinGroupId())) {
					break;
				} else {
					gcRsp.setRstType(GCResultType.DATA_ERROR);
					gcRsp.setTipMsg("已经开战，不可竞猜");
		...
		boolean enoughCoin = itemBagMgr.checkEnoughItem(player, eSpecialItemId.Coin.getValue(), coin);
		if (!enoughCoin) {
		...
		itemBagMgr.addItem(player, eSpecialItemId.Coin.getValue(), -coin);

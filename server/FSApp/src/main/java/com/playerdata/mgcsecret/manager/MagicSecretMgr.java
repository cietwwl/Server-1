...
public class MagicSecretMgr {
	...
	public void enterMSFight(Player player, MagicSecretRspMsg.Builder msRsp, String dungeonID) {
		...
		ArmyInfo enimyArmy = AngelArrayTeamInfoHelper.getInstance().parseTeamInfo2ArmyInfo(enterDungeon.getEnimyTeam());
		try {
			enimyArmy.genVCode();
	...
	public void getSingleReward(Player player, MagicSecretRspMsg.Builder msRsp, String dungeonID, String finishState) {
		...
		if (finishStar > 0) {
			...
			if (MSConditionJudger.fromStageIDToLayerID(stageID) == STAGE_COUNT_EACH_CHATPER)
				...
				handleNextDungeonPrepare(player, dungeonID);
		}
		// 清空刚战斗的关卡
	...
	public msResultType exchangeBuff(Player player, String chapterID, String buffID) {
		if (!MSConditionJudger.judgeBuffLegal(player, chapterID, buffID))
			return msResultType.DATA_ERROR;
		if (!MSConditionJudger.judgeEnoughStar(player, chapterID, buffID))
			return msResultType.NOT_ENOUGH_STAR;
	...
	public void openRewardBox(Player player, MagicSecretRspMsg.Builder msRsp, String chapterID, MSItemInfo msRwdBox) {
		...
		if (msRwdBox.getItemID().equalsIgnoreCase("1"))
			dropStr = dungDataCfg.getObjCoBox().getDropStr();
		if (msRwdBox.getItemID().equalsIgnoreCase("2"))
			dropStr = dungDataCfg.getObjHiBox().getDropStr();
	...
	public boolean addSecretGold(Player player, int secretGold) {
		if (secretGold < 0)
			return deductSecretGold(player, -secretGold);
		UserMagicSecretData umsData = UserMagicSecretHolder.getInstance().get(player);
	...
	public boolean hasScoreReward(Player player) {
		...
		for (DungeonScoreCfg cfg : dungScoreCfgList) {
			if (umsData.getGotScoreReward().contains(cfg.getKey()))
				continue;
			if (totalScore >= cfg.getScore())
				return true;
		}
		return false;
	}

...
public class ItemBagManager {
	 ...
	 * 消耗物品规则的比较器
	 */
	private final Comparator<ItemData> comparator = new Comparator<ItemData>() {
	...
	private ItemBagHolder holder = new ItemBagHolder();// 背包的数据层
	 ...
	 * @return
	 */
	public ItemData findBySlotId(Player player, String id) {
		return holder.getItemData(player, id);
	}
	 ...
	 * @return
	 */
	public int getItemCountByModelId(Player player, int modelId) {
		return holder.getItemCountByModelId(player, modelId);
	}
	...
	public void removeAllItems() {
		// Iterator<Entry<Integer, ItemData>> iter =
		// tableItem.getItemList().entrySet().iterator();
	...
	// HashMap<Integer, ConsumeCfg> cfglist =
	// ConsumeCfgDAO.getInstance().getEnhanceMap();
	...
	// /**
	// * 发送物品背包数据，响应消息不用每次都设置消息类型
	// response.setEventType(EItemBagEventType.ItemBag_Sync);应该设置到while循环外，只设置一次
	// *
	// * @param syncItemMap
	// */
	...
	//
	// // if (extendAttr != null) {
	// // Iterator<Entry<eItemAttrIdDef, String>> iter1 =
	// extendAttr.entrySet().iterator();
	// // while (iter1.hasNext()) {
	...
	public boolean addItem(Player player, int cfgId, int count) {
		return addItem0(player, cfgId, count, null, null, true);
	}
	...
	public boolean useItemBySlotId(Player player, String slotId, int count) {
		...
		return useItem0(player, slotId, count, null, null, true);
	}
	...
	public boolean useItemByCfgId(Player player, int cfgId, int count) {
		return useItemByCfgId0(player, cfgId, count, null, null, true);
	}
	 ...
	 * @return
	 */
	private boolean useItemByCfgId0(Player player, int cfgId, int count, List<INewItem> newItemList, List<IUpdateItem> updateItemList, boolean dataOperation) {
		if (count <= 0) {
			return false;
		}

		List<ItemData> itemList = holder.getItemDataByCfgId(player, cfgId);
		if (itemList.isEmpty()) {
			return false;
		}
		...
		return useItem0(player, itemList.get(0).getId(), count, newItemList, updateItemList, dataOperation);
	}
	...
	public boolean useLikeBoxItem(Player player, List<IUseItem> useItemList, List<INewItem> addItemList) {
		...
		if (useItemList != null && !useItemList.isEmpty()) {
			...
			for (int i = 0, size = useItemList.size(); i < size; i++) {
				...
				ItemData itemData = findBySlotId(player, slotId);
				if (itemData == null) {
					return false;
				}
			...
			for (Entry<String, Integer> e : tempMap.entrySet()) {
				if (!useItem0(player, e.getKey(), e.getValue(), newItemList, updateItemList, false)) {
					return false;
				}
			}
		}
		...
		if (addItemList != null && !addItemList.isEmpty()) {
			...
			for (Entry<Integer, Integer> e : tempMap.entrySet()) {
				if (!addItem0(player, e.getKey(), e.getValue(), newItemList, updateItemList, false)) {
					return false;
				}
			}
		}
	 ...
	 *            是否直接背包操作
	 */
	private boolean addItem0(Player player, int cfgId, int count, List<INewItem> newItemList, List<IUpdateItem> updateItemList, boolean bagOperation) {
	 ...
	 * @return 当前返回的只是一个状态，但是以后可能会返回失败的详细信息（这里要改成返回一个类型码）
	 */
	private boolean useItem0(Player player, String slotId, int count, List<INewItem> newItemList, List<IUpdateItem> updateItemList, boolean bagOperation) {
		if (count <= 0) {
			return false;
		}

		ItemData item = findBySlotId(player, slotId);// 查找对应的物品
		if (item == null) {
			return false;
		}
	 ...
	 * @return
	 */
	public List<ItemData> getItemListByCfgId(Player player, int cfgId) {
		return holder.getItemDataByCfgId(player, cfgId);
	}
	 ...
	 * @return
	 */
	public ItemData getFirstItemByModelId(Player player, int modelId) {
		return holder.getFirstItemByModelId(player, modelId);
	}
	 ...
	 * @param itemData
	 */
	public void updateItem(Player player, ItemData itemData) {
		holder.updateItem(player, itemData);
	}
	 ...
	 * @return
	 */
	public List<ItemData> getItemListByType(Player player, EItemTypeDef itemType) {
		return holder.getItemListByType(player, itemType);
	}

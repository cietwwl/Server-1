...
public class ItemBagHolder implements RecordSynchronization {
	 ...
	 * @return
	 */
	public ItemData getItemData(Player player, String id) {
		return getItemStore(player.getUserId()).getItem(id);
	}
	 ...
	 */
	public List<ItemData> getItemDataByCfgId(Player player, int modelId) {
	...
	public void updateItemBag(Player player, List<INewItem> newItemList, List<IUpdateItem> updateItemList) {
		...
		MapItemStore<ItemData> itemDataStore = getItemStore(userId);
		...
		if (updateItemList != null && !updateItemList.isEmpty()) {// 更新物品列表不为空
			for (int i = 0, size = updateItemList.size(); i < size; i++) {
				...
				ItemData itemData = this.getItemData(player, updateItem.getSlotId());
				if (itemData == null) {
					continue;
				}
	...
	public void updateItem(Player player, ItemData itemData) {
		getItemStore(player.getUserId()).updateItem(itemData);
	}
	...
	public void flush(Player player) {
		getItemStore(player.getUserId()).flush();
	}
	...
	public List<ItemData> getItemListByType(Player player, EItemTypeDef itemType) {
		...
		Enumeration<ItemData> allItem = getItemStore(player.getUserId()).getEnum();

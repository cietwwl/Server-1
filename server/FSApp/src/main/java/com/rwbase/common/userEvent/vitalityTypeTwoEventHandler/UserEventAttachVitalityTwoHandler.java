...
public class UserEventAttachVitalityTwoHandler implements IUserEventHandler{
	private List<UserEventHandleTask> eventTaskList = new ArrayList<UserEventHandleTask>();
	public UserEventAttachVitalityTwoHandler(){
		init();	
	}
	...
	private void init(){
		eventTaskList.add(new UserEventHandleTask() {
			...
			public void doAction(Player player, Object params) {
				ActivityVitalitySubCfg subCfg = ActivityVitalitySubCfgDAO.getInstance().getByTypeAndActiveType(ActivityVitalityTypeEnum.VitalityTwo,ActivityVitalityTypeEnum.AttachVitalityTwo.getCfgId());
				...
				if(subCfg!=null&&isLevelEnough){
					...
					ActivityVitalityTypeMgr.getInstance().addCount(player,ActivityVitalityTypeEnum.VitalityTwo, ActivityVitalityTypeEnum.AttachVitalityTwo,subCfg,add);
					GameLog.error(LogModule.ComActivityVitality, "userId:"+player.getUserId(), "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~活动之王-送体开启",null);
					}
				}
			@Override
			public void logError(Player player,Throwable ex) {
				StringBuilder reason = new StringBuilder(ActivityVitalityTypeEnum.AttachVitalityTwo.toString()).append(" error");				

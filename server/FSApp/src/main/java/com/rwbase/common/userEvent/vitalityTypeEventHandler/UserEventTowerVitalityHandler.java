	...
	private void init(){
		eventTaskList.add(new UserEventHandleTask() {
			...
			public void doAction(Player player, Object params) {
				ActivityVitalitySubCfg subCfg = ActivityVitalitySubCfgDAO.getInstance().getByType(ActivityVitalityTypeEnum.TowerVitality.getCfgId());
				
				boolean isLevelEnough = ActivityVitalityTypeMgr.getInstance().isLevelEnough(player);
				if(subCfg!=null&&isLevelEnough){
					ActivityVitalityTypeMgr.getInstance().addCount(player, ActivityVitalityTypeEnum.TowerVitality,subCfg, Integer.parseInt(params.toString()));

		...
		eventTaskList.add(new UserEventHandleTask() {
			...
			public void doAction(Player player, Object params) {
					...
					boolean isBetweendays = ActivityDailyTypeMgr.getInstance().isOpen(ActivityDailyTypeSubCfgDAO
							.getInstance().getListByEnumId(ActivityDailyTypeEnum.TreasureLandDaily.getCfgId()));
					boolean isLevelEnough = ActivityDailyTypeMgr.getInstance().isLevelEnough(player);
					if(isBetweendays&&isLevelEnough){
						ActivityDailyTypeMgr.getInstance().addCount(player, ActivityDailyTypeEnum.TreasureLandDaily,1);	
						
					}
				}
			@Override
			public void logError(Player player,Throwable ex) {
				StringBuilder reason = new StringBuilder(ActivityDailyTypeEnum.TreasureLandDaily.toString()).append(" error");				

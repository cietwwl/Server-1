		...
		eventTaskList.add(new UserEventHandleTask() {
			...
			public void doAction(Player player, Object params) {
				...
				if (instance.isOpenAndLevelEnough(
						player.getLevel(),
						instance.getCfgMapByEnumid(ActivityDailyTypeEnum.TreasureLandDaily
								.getCfgId()))) {
					ActivityDailyTypeMgr.getInstance().addCount(player,
							ActivityDailyTypeEnum.TreasureLandDaily, 1);
				}
			}
			...
			public void logError(Player player, Exception ex) {
				StringBuilder reason = new StringBuilder(
						ActivityDailyTypeEnum.TreasureLandDaily.toString())
						.append(" error");

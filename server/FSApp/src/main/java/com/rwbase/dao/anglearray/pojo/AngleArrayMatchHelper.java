...
public final class AngleArrayMatchHelper {
	 ...
	 * @return
	 */
	public static ArmyInfo getMatchArmyInfo(String userId, int level, int minFighting, int maxFighting) {
		// 第一步等级匹配 && 第二步战力匹配
		List<String> matchUsetIdList = getMatchUsetIdList(userId, level, minFighting, maxFighting);
		...
		if (needFightingRankCount > 0) {
			getFightingRankMatch(userId, needFightingRankCount, matchUsetIdList);
		}
	 ...
	 * @return
	 */
	private static void getFightingRankMatch(String userId, int rankCount, List<String> hasMatchList) {
		Ranking rank = RankingFactory.getRanking(RankType.FIGHTING_ALL_DAILY);
		if (rank == null) {
			return;
		}

		int maxRanking = rank.size();// 最后一条记录的数据

...
public class MagicEquipConditionCfg {
	...
	private Map<Integer, Map<Integer, Integer>> conditionMap;// 条件集合<key=modelID,
																// value=条件集合>
	private MagicEquipConditionKey compositeKey;
	...
	public void formateData() {
		...
		compositeKey = new MagicEquipConditionKey(type, subType);
		...
		if (ids.length != str.length) {
			throw new ExceptionInInitializerError(String.format("唯一Id[%s]解析法宝神器羁绊表，条件不对应，表中的id数[%s]，条件值长度[%s]", uniqueId, ids.length, str.length));
		}
		...
		for (int i = 0; i < ids.length; i++) {
			...
			if (type == FetterMagicEquipCfgDao.TYPE_MAGICWEAPON
					&& (conditionMap.containsKey(FettersBM.SubConditionType.STAR.type) || conditionMap.containsKey(FettersBM.SubConditionType.QUALITY.type) || conditionMap
							.containsKey(FettersBM.SubConditionType.FIGHTING.type))) {

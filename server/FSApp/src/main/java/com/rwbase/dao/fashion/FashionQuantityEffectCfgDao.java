...
public class FashionQuantityEffectCfgDao extends CfgCsvDao<FashionQuantityEffectCfg> {
	...
	public Map<String, FashionQuantityEffectCfg> initJsonCfg() {
		Map<String, FashionQuantityEffectCfg> readCsv2Map = CfgCsvHelper.readCsv2Map("fashion/FashionQuantityEffectCfg.csv", FashionQuantityEffectCfg.class);

		if (readCsv2Map != null && !readCsv2Map.isEmpty()) {
			Collection<Entry<String, FashionQuantityEffectCfg>> values = readCsv2Map.entrySet();

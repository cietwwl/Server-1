...
public class PlayerQuestionService {
				...
				sendService.submit(new Runnable() {
					...
					public void run() {
						try {
							...
							if (obj.requireList) {
								List send2 = borrowSender.send2(obj.getContent(), obj.getResponseClass(), obj.getOpType());
								playerQuestionMgr.processResponse(send2, obj.getOpType());
							} else {
								Object send = borrowSender.send(obj.getContent(), obj.getResponseClass(), obj.getOpType());
								playerQuestionMgr.processResponse(send, obj.getOpType());
							}
						} catch (Exception e) {
	...
	public <T> void submitRequest(Map<String, Object> content, int opType, String userId, boolean requireList, Class<T> responseClass){
		QuestionObject obj = new QuestionObject(content, opType, userId, requireList, responseClass);
		Queue.add(obj);
	}

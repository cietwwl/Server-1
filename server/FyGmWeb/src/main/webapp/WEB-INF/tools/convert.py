...
def sheet2json(subfolderpath, subfolder, sheet, jsonfile):
    ...
    try:
        ...
        if os.path.exists(path)==False:
            os.makedirs(path)
        file = codecs.open(path+jsonfile, "w", "utf-8")
        file.write("%s" % json.dumps(resultlist, ensure_ascii=False, indent=4))
        file.close()
    #except TypeError, e:
...
def processXls(xls_file):
    ...
    if xls_file!="":
        ...
        for sheet in workbook.sheets():
            ...
            print u"Converting %s's %s from EXCEL's sheet to JSON as %s" % (folder_name, sheet.name, jsonfile)

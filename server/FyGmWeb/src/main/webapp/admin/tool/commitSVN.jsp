...
<body>
<center>
<form action='<%=response.encodeURL(request.getContextPath() + "/admin/updatesvn.a")%>' enctype="multipart/form-data" id="form" method="post">
	<div style="height:30px;float:center;margin-right:8px;">
		<p>选择要提交的svn并提交svn文件</p>
		<label>SVN服务器组</label>
		<select id="server" name="svnId" style="width:200px">
			<s:iterator value="svnList" status="st">
			<option value=<s:property value="svnId" /> selected="selected"><s:property value="svnName" /></option>
			</s:iterator>
		</select>
		<input type="file" name="file" id="file" onpropertychange="show();" />

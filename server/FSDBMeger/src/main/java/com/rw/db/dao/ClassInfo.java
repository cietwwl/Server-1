...
public class ClassInfo {
	...
	public String genUpdateSQL(){
		...
		for (int i = 0, len = columns.length; i < len; i++) {
			...
			addSplit(updateFields).append(columnName).append("=?");
		}
		
		return "update " + tableName + " set " + updateFields.toString() + " where " + getPrimaryKey() + " = ?";
	}
	
	public String genDeleteSQL(){
		return "delete from " + tableName + " where " + getPrimaryKey() + " = ?";
	}
